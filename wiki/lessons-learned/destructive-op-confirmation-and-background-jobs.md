...
- [A Shared-File Collision Between Concurrent Agents Is a Protocol Gap, Not a Latency Gap](concurrent-session-shared-file-collision.md) — the "serialize writes to a shared store" principle recurs here as a real production outage caused by two independent agents editing the same config file without a claim-before-edit signal
