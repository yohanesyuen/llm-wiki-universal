...
## [2026-08-09] lint | 3 issues found, 1 auto-fixed
Sweep verification after the mechanical frontmatter normalisation (81 articles here: keys lowercased to the closed set, unquoted markdown links quoted, `;`-joined quoted scalars converted to YAML lists). All 82 articles parse as YAML; 0 capitalised keys; 0 keys outside `type/tags/title/sources/raw/updated`. Verified independently of the sweep's own report that the conversion lost nothing: per-file frontmatter `](`-count and post-frontmatter body text both compared against `HEAD` — 0 link-count mismatches, 0 bodies changed across all 81. Every `raw:` target and every in-article internal link resolves.
- Fixed: `log.md`'s 2026-06-29 ingest entry linked its raw source as `raw/...` (project-root-relative) from inside `wiki/`, so it never resolved — corrected to `../raw/...`.
